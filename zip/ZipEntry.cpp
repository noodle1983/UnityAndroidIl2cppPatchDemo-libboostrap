...
bool ZipEntry::compareHeaders(void) const
{
    ...
    if (hasDD()){
        if (mDD.mCRC32 != mLFH.mCRC32) {
            ALOGV("cmp: CRC32, dd:\n");
            return false;
        }
        if (mDD.mCompressedSize != mLFH.mCompressedSize) {
            ALOGV("cmp: CompressedSize\n");
            return false;
        }
        if (mDD.mUncompressedSize != mLFH.mUncompressedSize) {
            ALOGV("cmp: UncompressedSize\n");
            return false;
        }
    }
...
/*
 * ===========================================================================
 *      ZipEntry::DataDescriptor
 * ===========================================================================
 */

status_t ZipEntry::DataDescriptor::read(FILE* fp)
{
    status_t result = NO_ERROR;
    ...
    mCRC32 = ZipEntry::getLongLE(&buf[0x4]);
    mCompressedSize = ZipEntry::getLongLE(&buf[0x8]);
    mUncompressedSize = ZipEntry::getLongLE(&buf[0xc]);
 ...
 * Write a central dir entry.
 */
status_t ZipEntry::DataDescriptor::write(FILE* fp)
{
    unsigned char buf[kDDLen];

    ZipEntry::putLongLE(&buf[0x00], kSignature);
    ZipEntry::putLongLE(&buf[0x4], mCRC32);
    ZipEntry::putLongLE(&buf[0x8], mCompressedSize);
    ZipEntry::putLongLE(&buf[0xc], mUncompressedSize);

    return NO_ERROR;
}

/*
 * Dump the contents of a DataDescriptor object.
 */
void ZipEntry::DataDescriptor::dump(void) const
{
    ALOGD(" DataDescriptor contents:\n");
    ALOGD("  crc32=0x%08lx\n", mCRC32);
    ALOGD("  compressedSize=%lu uncompressedSize=%lu\n",
        mCompressedSize, mUncompressedSize);
}
